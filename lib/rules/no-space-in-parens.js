...
module.exports = {
    ...
    create(context) {
        ...
        function checkStringInParen(node, prevToken, currentTemplateLiteral, nextToken) {
            if (sourceCode.isSpaceBetweenTokens(prevToken, currentTemplateLiteral.startToken)) {
                context.report({
                    node,
                    loc: currentTemplateLiteral.startToken.loc.start,
                    message: UNNECESSARY_SPACE_MESSAGE,
                    fix(fixer) {
                        return fixer.removeRange([prevToken.end, currentTemplateLiteral.startToken.start]);
                    }
                });

            }

            if (sourceCode.isSpaceBetweenTokens(currentTemplateLiteral.endToken, nextToken)) {
                context.report({
                    node,
                    loc: nextToken.loc.start,
                    message: UNNECESSARY_SPACE_MESSAGE,
                    fix(fixer) {
                        return fixer.removeRange([currentTemplateLiteral.endToken.end, nextToken.start]);
                    }
                });
        ...
        function stringInParenAlreadyCheckedTemplateLiterals(tokens, i, templateLiteralsСoordinates) {
            if (!tokens[ i ]) {
                return false;
            }
            ...
            if (nextToken && nextToken.type === "Template") {
                const coordinatePair = templateLiteralsСoordinates.filter(coordinates => {
                    return coordinates[0].index === i + 1 || coordinates[1] && coordinates[1].index === i + 1;
                })[0];
                const isSingleTemplateLiteral = coordinatePair.length === 1;

                if (!isSingleTemplateLiteral) {
                    nextNextToken = tokens[coordinatePair[1].index + 1];
                }
            }
            ...
            if (prevToken && prevToken.type === "Template") {
                ...
                if (!isSingleTemplateLiteral) {
                    prevPrevToken = tokens[coordinatePair[0].index - 1];
                }
            }

            if (token && token.type == "Template") {
                const coordinatePair = templateLiteralsСoordinates.filter(coordinates => {
                  return coordinates[0].index === i || coordinates[1] && coordinates[1].index === i;
                })[0];
                const isSingleTemplateLiteral = coordinatePair.length === 1;
  
                if (!isSingleTemplateLiteral) {
                    prevToken = tokens[coordinatePair[0].index - 1];
                    nextToken = tokens[coordinatePair[1].index + 1];
                }
            }
        ...
        /**
         * Defines the coordinates of beginning and end indexes for template literals
         * @param {Array} tokens All tokens from eslint parser
         * @returns {Array} Array of chunks with beginning and end indexes for template literals
         */
        function getTemplateLiteralsСoordinates(tokens) {
            const templateLiterals = tokens.reduce((acc, token, index) => {
            ...
            const templateLiteralsChunks = templateLiterals.reduce((acc, templateLiteral) => {
        ...
        return {
            Program: function checkParenSpaces(node) {
                ...
                const templateLiteralsСoordinates = getTemplateLiteralsСoordinates(tokens);
                ...
                tokens.forEach( function( token, i ) {
                     ...
                     * 
                     * If the token is single, startToken and endTokens are same
                     * 
                    ...
                    */
                    ...
                    if (
                        ...
                        checkStringInParen(node, prevToken, currentTemplateLiteral, nextToken);
                        return;
                    }

                    if (
                      options.parenStringException &&
                      (stringInParenAlreadyChecked(tokens, i) || stringInParenAlreadyCheckedTemplateLiterals(tokens, i, templateLiteralsСoordinates))
                    ) {
                        return;
                    }
